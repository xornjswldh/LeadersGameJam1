...
function C.create(card,pos,character,title,effect,cost,hold)
	local new_card={node=card,pos=pos,character=character,title=title,effect=effect,cost=cost,hold=false}
	...
	function new_card:use()
		msg.post(CM, "use_card",{character=new_card.character,title=new_card.title,effect=new_card.effect,cost=new_card.cost})
